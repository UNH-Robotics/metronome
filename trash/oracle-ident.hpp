...
template <class D>
class Oracle_Ident : public SearchAlgorithm<D> {
private:
...
public:
    ...
    Oracle_Ident(int argc, const char *argv[]) :
    ...
    virtual void output(FILE *out) {
        ...
        dfpair(out, "number of switches taken", "%u", nswitch);
...
private:
    ...
    std::pair<Node*, double> move(D &d, Node *cur, LssNode *goal) {
        ...
        if (!best) {
            for (auto n : lssOpen.data()) {
                ...
                if (best == NULL)
                    best = n;
                else if(LssNode::FHatSort::pred(n, best))
                {
                    secondBest = best;
                    best = n;
                }
                else if( secondBest == NULL || LssNode::FHatSort::pred(n, secondBest) )
                    secondBest = n;
            }
        }
        assert (best);
        ...
        if (!goal && cur->ident.first != D::Nop && computeBenefit(d, cur, best, secondBest, cur->ident.second) > cur->ident.second )
        {
    ...
    double computeBenefit(D &d, Node *n, LssNode *alpha, LssNode *beta, double g) {
        ...
        double aFStar, bFStar;

        Astar<D> *astar;
        State state;
        if( alpha->node->hstar < 0.0 )
        {
            astar = new Astar<D>(numArgs, args);
            d.unpack( state, alpha->node->state );
            astar->search( d, state );
            alpha->node->hstar = astar->getCost( );
            delete astar;
        }
        if( beta->node->hstar < 0.0 )
        {
            astar = new Astar<D>(numArgs, args);
            d.unpack( state, beta->node->state );
            astar->search( d, state );
            beta->node->hstar = astar->getCost( );
            delete astar;
        }

        aFStar = alpha->node->hstar + alpha->g;
        bFStar = beta->node->hstar + beta->g;

        double mu_alpha = aFStar, mu_beta = bFStar;

        double bv_alpha = pow(aFStar - alpha->f, 2),
                bv_beta  = pow(bFStar - beta->f, 2);
        double x = lsslim->lookahead(g) / (delaySum / delayCount);

        //TODO if not only considering identity actions, fix this
        double v_alpha = bv_alpha * (x / alpha->node->d), v_beta = bv_beta * (x / beta->node->d);
    ...
    unsigned int nshort, nident, nswitch;    // Number of short and identity trajectories taken.
