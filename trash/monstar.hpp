// MON* based on the Lsslrtastar2 implementation.
...
template <class D>
class Monstar : public SearchAlgorithm<D> {
private:
...
public:
	...
	Monstar(int argc, const char *argv[]) : SearchAlgorithm<D>(argc, argv), lssNodes(1024), nodes(30000001) {
	...
	void search(D &d, State &s0) {
		...
		while (!cur->goal && !this->limit()) {
			LssNode *goal = expandLss(d, cur, initialSearch);
...
private:
	...
	// and NULL otherwise.
	LssNode *expandLss(D &d, Node *rootNode, bool initialSearch) {
		lssOpen.clear();
		lssNodes.clear();
		lssPool.releaseall();
		nclosed = 0;
		...
		while (!lssOpen.empty() && (!lsslim->stop() || !initialSearch) && !this->limit()) {
