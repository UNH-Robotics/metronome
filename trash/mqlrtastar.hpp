...
public:
    ...
    virtual bool stop() = 0;
    virtual unsigned long lookahead( double ) = 0;
...
template <class D>
class MQlrtastar : public SearchAlgorithm<D> {
private:
...
public:
    ...
    MQlrtastar(int argc, const char *argv[]) :
    ...
    std::pair<Node*, double> move(Node *cur, LssNode *goal) {
        ...
        if (!best) {
            ...
            for (auto n : lssOpen.data()) {
                ...
                if (n->node == cur)
                    continue;
                if (best == NULL || n->f < best->f || (!random && geom2d::doubleeq(n->f, best->f) && n->g > best->g))
                {
                    best = n;
                    count = 0;
                }
                if(random && geom2d::doubleeq(n->f, best->f) && rand() % ++count)
                    best = n;
            }
        }
        assert (best);
...
public:
    ...
    virtual unsigned long lookahead( double ){ return lim; }
...
public:
    ...
    virtual unsigned long lookahead( double g )
    {
        return g * gscale;
    };
    ...
    virtual void output(FILE *out) const {
        ...
        dfpair(out, "expansions per unit duration", "%f", gscale);
