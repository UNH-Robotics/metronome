...
template <class D>
class daLsslrtastar : public SearchAlgorithm<D> {
private:
...
public:
	...
	daLsslrtastar(int argc, const char *argv[]) :
	...
	std::pair<Node*, double> move(Node *cur, LssNode *goal) {
		...
		if (!best) {
			for (auto n : lssOpen.data()) {
				if (n->node == cur)
					continue;
				if (best == NULL || (best->node->h - best->node->h0) > (n->node->h - n->node->h0))
					best = n;
			}
		}
		assert (best);
