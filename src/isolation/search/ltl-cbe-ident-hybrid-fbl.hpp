...
template <class D>
class LTL_CBE_IDENT_HYBRID_FBL : public SearchAlgorithm<D> {
private:
...
public:
    ...
    LTL_CBE_IDENT_HYBRID_FBL(int argc, const char *argv[]) :
...
private:
    ...
    double computeBenefit(D &d, Node *n, LssNode *alpha, LssNode *beta, double g) {
        ...
        double bv_alpha = pow(std::max( 0.0, alpha->f - lssClosed.find(n->state)->f) / std::abs( n->d - alpha->node->d ) * alpha->node->d, 2),//pow(alpha->fhat - alpha->f, 2),
                bv_beta  = pow(std::max( 0.0, beta->f - lssClosed.find(n->state)->f) / std::abs( n->d - alpha->node->d ) * beta->node->d, 2);//pow(beta->fhat - beta->f, 2);
