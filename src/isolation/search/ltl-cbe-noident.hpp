...
template <class D>
class LTL_CBE_NOIDENT : public SearchAlgorithm<D> {
private:
...
public:
    ...
    LTL_CBE_NOIDENT(int argc, const char *argv[]) :
        ...
        if( !goal )
        {
            ...
            do
            {
                ...
                for (auto n : lssOpen.data()) {
                    ...
                    if (alpha == NULL)
                        alpha = n;
                    else if(LssNode::FHatSort::pred(n, alpha))
                    {
                        beta = alpha;
                        alpha = n;
                    }
                    else if( beta == NULL || LssNode::FHatSort::pred(n, beta) )
                        beta = n;
                }
            }
