...
template <class D>
class Fhatlrtastar3 : public SearchAlgorithm<D> {
private:
    ...
    public:
        ...
        bool expd, newExpansion;  // was this expanded before?
        bool goal;
    ...
    private:
        ...
        Node() : expd(false), newExpansion(true) {
        }
...
public:
    ...
    Fhatlrtastar3(int argc, const char *argv[]) :
    ...
    LssNode *expandLss(D &d, Node *rootNode) {
        ...
        while (!lssOpen.empty() && !lsslim->stop() && !this->limit()) {
            ...
            for (auto e : expand(d, s->node)) {
                ...
                if (!s->node->newExpansion && (!bestkid || kid->f < bestkid->f))
                    bestkid = kid;

                s->node->newExpansion = false;
            }
            ...
            if (bestkid) {
                ...
                if (herr < 0)
                    herr = 0;
                herrnext += (herr - herrnext)/(++bestKids);
                ...
                derrnext += (derr - derrnext)/(bestKids);
            }
