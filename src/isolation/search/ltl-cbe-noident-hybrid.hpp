...
template <class D>
class LTL_CBE_NOIDENT_HYBRID : public SearchAlgorithm<D> {
private:
...
public:
    ...
    LTL_CBE_NOIDENT_HYBRID(int argc, const char *argv[]) :
...
private:
    ...
    std::pair<Node*, double> move(D &d, Node *cur, LssNode *goal) {
        ...
        if(!goal)
        {
            ...
            for( LssNode *p : path )
            {
                ...
                double area = ( std::max( alpha->depth, beta->depth ) - to->depth )
                                                        * ( delaySum / delayCount );
    ...
    double computeBenefit(D &d, Node *n, LssNode *alpha, LssNode *beta, double g) {
        ...
        delay_beta += 1 + expCount +
                     ...
                     (std::abs(alpha->fhat - lssClosed.find(n->state)->fhat) /
                      alpha->genCount) - beta->genCount;

        delay_beta /= i;

        LssNode *cur = lssClosed.find( n->state );

        double bv_alpha = pow((cur->f - alpha->f)
                            / ( alpha->depth - cur->depth )
                            * alpha->node->d, 2),
               bv_beta  = pow((cur->f - beta->f)
                            / ( beta->depth - cur->depth )
                            * beta->node->d, 2);

        double mu_alpha = alpha->f + sqrt(bv_alpha),
               mu_beta = beta->f + sqrt(bv_beta);

        double x_alpha = std::min(alpha->node->d, x / delay_alpha),
               x_beta = std::min(beta->node->d,   x / delay_beta);

        //TODO if not only considering identity actions, fix this
        double v_alpha = bv_alpha * (x_alpha / alpha->node->d),
               v_beta = bv_beta * (x_beta / beta->node->d);
        ...
        for( int i = 0; i < 100; ++i )
        {
            ...
            benefit += sum * std::exp(-pow(a-mu_alpha,2)/(2 * v_alpha))/(sqrt(2 * PI * v_alpha)) * ((end - start)/100.0);
        }
