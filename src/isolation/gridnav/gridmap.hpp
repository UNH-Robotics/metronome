...
struct GridMap {
	...
	bool inbounds(int loc, const Move &m) const {
		for (unsigned int i = 0; i < m.n; i++) {
			int nxt = loc + m.chk[i].delta;
			std::pair<int,int> c = coord(nxt);
			unsigned int x = c.first;
			unsigned int y = c.second;
			if(x <= 0 || y <= 0 || x >= (unsigned int)(w-1) || y >= (unsigned int)(h-1))
				return false;
		}
		return true;
	}
...
private:
	...
	// load_dylan loads a map with hardcoded heuristic estimates used primarily
	// for analysis of metareasoning agents.
	void load_dylan(FILE *);
