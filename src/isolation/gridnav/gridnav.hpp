...
struct GridNav {
	...
	public:
		...
		static Operators allOps(const GridNav &d) {
			Operators all;
			for (unsigned int i = 0; i < d.map->nmvs; i++)
				all.ops[all.n++] = i;
			return all;
		...
		static Operators inboundsOps(const GridNav &d, const State &s) {
			Operators inbounds;
			for (unsigned int i = 0; i < d.map->nmvs; i++) {
				if (d.map->inbounds(s.loc, d.map->mvs[i]))
					inbounds.ops[inbounds.n++] = i;
			}
			return inbounds;
		}

		bool applicable(const GridNav &d, const State &s, unsigned int op) const {
			return d.map->ok(s.loc, d.map->mvs[ops[op]]);
		}
	...
	struct Edge {
		...
		Edge(const GridNav &d, const State &s, const Oper op) {
	...
	virtual Cost pathcost(const std::vector<State>&, const std::vector<Oper>&, bool ) const;
...
private:
	...
	std::pair<Cost, int> hardcoded( int s ) const
	{
	    return std::make_pair( Cost( ), 0 );
	}
