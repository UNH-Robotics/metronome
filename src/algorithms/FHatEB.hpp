...
public:
    Fhatlrtastar(int argc, const char* argv[])
            : SearchAlgorithm<D>(argc, argv), lssNodes(4051), herror(0), derror(0), nodes(30000001), onestep(false) {
...
private:
    ...
    void hCostLearning(D& d) {
        ...
        while (nclosed > 0 && !open.empty()) {
            LssNode* source = *open.pop();

            nclosed -= source->closed;

            for (auto e : source->node->preds) {
                Node* predecessorNode = e.node;
