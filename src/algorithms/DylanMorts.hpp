#ifndef METRONOME_DYLANMORTS_HPP
#define METRONOME_DYLANMORTS_HPP
...
class MORTS : public SearchAlgorithm<Domain> {
private:
    typedef typename Domain::PackedState PackedState;
    typedef typename Domain::Operators Operators;
    typedef typename Domain::State State;
    typedef typename Domain::Cost Cost;
    typedef typename Domain::Oper Oper;
    typedef typename Domain::Edge Edge;
    ...
    class Node {
    public:
        static ClosedEntry<Node, Domain>& closedentry(Node* n) { return n->closedent; }
    ...
    private:
        ...
        ClosedEntry<Node, Domain> closedent;
    };
    ...
    private:
        ClosedList<Node, Node, Domain> tbl;
        Pool<Node> pool;
    };
    ...
    public:
        ...
        class Nodes {
        public:
            static ClosedEntry<LssNode, Domain>& closedentry(LssNode* n) { return n->nodesent; }
    ...
    private:
        ClosedEntry<LssNode, Domain> nodesent;
    };

public:
    MORTS(int argc, const char* argv[])
            : SearchAlgorithm<Domain>(argc, argv),
              lssClosed(4051),
              herror(0),
              derror(0),
              nodes(30000001),
    ...
    virtual void output(FILE* out) {
        SearchAlgorithm<Domain>::output(out);
...
private:
    ...
    // and NULL otherwise.
    LssNode* expandLss(Domain& d, Node* rootNode, bool ident) {
    ...
    std::pair<Node*, double> move(Domain&domain, Node* cur, LssNode* goal) {
        LssNode* best = goal;
        LssNode* secondBest = NULL;

        // Find the two most promising nodes on the search frontier.
        // This looks suboptimal. Would it be sufficient to check the first two levels of the heap?
        // The top is the best and one of its children. (If ordered properly)
        ...
        // Skip if goal is found
        if (!goal) {
            if (cur->ident.first != Domain::Nop &&
                    computeBenefit(domain, cur, best, secondBest, cur->ident.second) > cur->ident.second) {
...
double computeBenefit(Domain& domain, Node* node, LssNode* alpha, LssNode* beta, double g) {
    // If there is no alternative the benefit is 0
    if (beta == NULL || alpha == beta) {
        return 0.0;
    }

    LssNode* source = lssClosed.find(node->state);
    ...
    int i = 1;
    double delay_alpha = 1 + expCount - alpha->genCount; // Include last step
    ...
    i = 1;
    double delay_beta = 1 + expCount - beta->genCount; // Include last step
    ...
    // ???
    delay_beta += (beta->fhat - alpha->fhat) / (std::abs(alpha->fhat - source->fhat) / alpha->genCount); // What?
    ...
    double bvariance_alpha = pow((source->f - alpha->f) / std::abs(alpha->depth - source->depth) * alpha->node->d, 2);
    ...
    // ???
    double x_alpha = std::min(alpha->node->d, x / delay_alpha);
    double x_beta = std::min(beta->node->d, x / delay_beta);

    // TODO if not only considering identity actions, fix this
    // Variance' (eq. 3)
    double variance_alpha = bvariance_alpha * (x_alpha / alpha->node->d);
    double variance_beta = bvariance_beta * (x_beta / beta->node->d);

    double alpha_lower = mu_alpha - 2.0 * sqrt(variance_alpha);
    double beta_lower = mu_beta - 2.0 * sqrt(variance_beta);
    double start = std::min(alpha_lower, beta_lower);

    double alphaHigher = mu_alpha + 2.0 * sqrt(variance_alpha);
    double betaHigher = mu_alpha + 2.0 * sqrt(variance_beta);
    double end = std::min(alphaHigher, betaHigher);

    // Integration step
    double benefit = 0.0;
    double step = (end - start) / 100.0;

    for (double a = 0.5 * step; a < end; a += step) {
        double sum = 0.0;
   ...
   //     sum *= step; REMOVED
        ...
        // What? multipy sum with step twice?
        benefit += sum * normalPDF(mu_alpha, variance_alpha, a); // * step; REMOVED
    }
    ...
    std::vector<outedge> expand(Domain& d, Node* n) {
        this->res.expd++;
    ...
    ClosedList<typename LssNode::Nodes, LssNode, Domain> lssClosed;
...
#endif // METRONOME_DYLANMORTS_HPP
